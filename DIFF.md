...
- **额度金额估算**：Codex 的 5h/7d 窗口从同一 usage 响应头分别推进本地观察区间；月窗口以及 Claude、Kimi、Antigravity、xAI 的 5h/7d/月窗口按额度刷新快照建立区间。只有覆盖面等于凭证整体用量的窗口才参与估算——Claude 仅 `five_hour`/`seven_day` 聚合窗口，Opus/Sonnet 等模型或应用子限额保持纯展示，避免用子集百分比去除全模型聚合成本；同时仍要求百分比、重置时间、周期齐备并启用 usage stream，5h 至少需要 2 个百分点增量，长窗口至少需要 3 个百分点。`quota_observed` 的旧 7d `secondary_*` JSON 可读，首次新观察写入时迁至按周期存储的 `windows`；界面按资源池拆分展示精确 5h+7d 双窗口，并按「池优先」排序，内联额度内优先保证每个池有一个代表窗口，其余进入 `+N`。
- **Codex 重置卡**：重置卡随额度刷新一并读取——同一次刷新里由 Codex 适配器先 GET `/wham/usage`、再 GET `/wham/rate-limit-reset-credits`，结果写入额度快照的 `reset_credits`（仅保留 available、未过期，按过期升序），界面直接从已同步的凭证数据渲染，不再有独立的查询/请求；读取失败写入 `reset_credits_failed` 标记，界面显示“上次刷新未能读取”而不是伪装成无卡。消费仍为二次确认 + 固定卡 ID + 全局唯一领取记录（account + credit 的 SHA-256），POST 前用实时列表重新校验最早卡、消费成功后立即刷新额度；读取类调用（provider GET，含额度/卡片/auth-files 同步）统一带 3 次有界重试（250ms 退避），写操作（消费 POST、凭证开关 PATCH）绝不自动重试。实例级自动使用默认关闭，且**依赖「凭证自动管理」**（只有它的巡检会按周期刷新额度与重置卡；设置界面在依赖未开启时直接禁用该开关，服务端在归一化与运行时同样强制该约束）。它不再单独定时扫描，而是在该实例的 runtime 周期里**跟在额度刷新/巡检之后**执行，候选直接取自刷新刚写入的快照（每张卡消费前仍实时复核一次，失败只跳过该张），仅消费未来 30 分钟内的卡。
