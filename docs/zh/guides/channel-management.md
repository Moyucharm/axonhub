...
## 配置多个 API Key（单 Key / Key 池）

当一个账号有多个 API Key 时，可以都配置到同一个渠道中。AxonHub 支持两种 Key 模式：

- **单 Key（single）**：渠道只保留一个 API Key，适合单账号单密钥场景。
- **多 Key 池（pool）**：渠道保留多个 API Key，AxonHub 自动轮换使用，提高稳定性。

### 创建时配置 Key 池

创建渠道时，在 **Key 模式** 中选择“多 Key 池”，然后在 **API Key** 区域逐行粘贴多个 Key（每行一个）即可；池内的重试与自动检测选项可一并配置（见下文）。

### 管理现有渠道的 Key 池

渠道创建后，通过 **管理密钥池** 入口维护存量 Key（渠道行菜单 → 管理密钥池；或编辑渠道 → 摘要卡片 → 管理密钥池）：

- **导入**：粘贴文本批量导入 Key（支持换行、逗号、分号或空格分隔，自动去重）
- **导出**：将当前筛选状态（全部/可用/已禁用）下的 Key 导出为 .txt 文件
- **检测**：校验 Key 可用性；勾选部分 Key 时仅检测选中的 Key
- **启用/禁用**：支持单个或批量启用、禁用（禁用需二次确认），也支持一键全部启用
- **删除**：从渠道中永久移除选中的 Key（需二次确认）
- **池设置**：配置池内重试次数、自动检测开关、检测间隔、并发与超时，并查看上次自动检测时间

编辑渠道时，Key 池不会被表单覆盖：存量 Key 统一在管理面维护，编辑页只显示可用/总数摘要。

### 从 Key 池切换回单 Key

编辑渠道时，将 Key 模式从“多 Key 池”切换到“单 Key”，系统会要求选择保留哪一把 Key，保存后其余 Key 会从渠道中移除。

### 负载均衡说明

- 相同的 Trace ID 会始终使用同一个 Key（保证会话一致性）
- 不同请求会随机选择可用的 Key
- 某个 Key 出错时，系统会自动切换到其他 Key（池内重试次数可在池设置中调整）

### 自动禁用与自动检测

- 连续失败的 Key 会被自动禁用，并记录失败次数、错误码与最近失败时间；已禁用的 Key 会显示禁用原因与过期时间（如有）
- 启用“自动检测”后，系统会按设置间隔（**最小 1 小时**）自动检测已禁用的 Key，恢复可用的 Key
- 池设置中可调整检测并发数与单 Key 超时时间
...
在渠道编辑界面将 **Key 模式** 设为“多 Key 池”，逐行添加多个 Key；渠道创建后可通过 **管理密钥池** 导入、检测和管理存量 Key。

### Q: API Key 被禁用了怎么恢复？

进入 **管理密钥池**，在“已禁用”筛选下找到该 Key，点击启用（恢复）按钮即可；也可以开启 **自动检测**，让系统定时自动恢复可用的 Key。
