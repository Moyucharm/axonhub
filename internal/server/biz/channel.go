...
func (svc *ChannelService) RegisterScheduledTasks(ctx context.Context, s *scheduler.Scheduler) error {
	...
	// Ticks every minute so disable_until_cron rules recover within the
	// finest granularity a crontab expression can express.
	if err := s.Register(ctx, scheduler.TaskSpec{
...
func (svc *ChannelService) UpdateChannel(ctx context.Context, id int, input *ent.UpdateChannelInput) (*ent.Channel, error) {
	...
	// Snapshot provider identity and server-managed credential state before
	// normalizing settings. Settings-only Command Code updates need this snapshot
	// to reject a concurrent type change, while credential edits must not accept
	// client-supplied API key operational state.
	existingIdentity, err := authz.RunWithScopeDecision(ctx, scopes.ScopeWriteChannels, func(queryCtx context.Context) (*ent.Channel, error) {
		return svc.entFromContext(queryCtx).Channel.Query().
			...
			Select(channel.FieldType, channel.FieldBaseURL, channel.FieldUpdatedAt, channel.FieldCredentials, channel.FieldPolicies).
			Only(queryCtx)
	})
	if err != nil {
	...
	if input.Credentials != nil {
		...
		input.Credentials.APIKeyStates = slices.Clone(existingIdentity.Credentials.APIKeyStates)
		...
		if existingIdentity.Credentials.IsAPIKeyPool() && !input.Credentials.IsAPIKeyPool() {
			if input.Policies == nil {
				policies := existingIdentity.Policies
